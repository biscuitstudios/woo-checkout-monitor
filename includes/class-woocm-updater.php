...
class Woocm_Updater {
    ...
    const REPO = 'biscuitstudios/woo-checkout-monitor';

    const CACHE_KEY = 'woocm_updater_release_v1';
    ...
    private function fetch_release() {
        ...
        if ( ! is_array( $body ) || [] === $body ) {
            error_log( '[BSM] Update check failed: no releases listed for ' . self::REPO );
        ...
        if ( [] === $log ) {
            error_log( '[BSM] Update check failed: no usable release tags for ' . self::REPO );
        ...
        if ( '' === $package ) {
            error_log( '[BSM] Update check failed: release v' . $latest['version'] . ' has no zip asset.' );
    ...
    private function render_changelog( $release ) {
        ...
        foreach ( $release['log'] as $entry ) {
            ...
            $out .= '' === $notes
                ? '<p>' . esc_html__( 'No release notes were published for this version.', 'woo-checkout-monitor' ) . '</p>'
        ...
        $out .= sprintf(
            ...
            $release['truncated']
                ? esc_html__( 'Earlier releases are on GitHub', 'woo-checkout-monitor' )
                : esc_html__( 'View all releases on GitHub', 'woo-checkout-monitor' )
        );
